...
class EncryptionConfigError(RuntimeError):
    ...
    Caller (lifespan startup or first PHI write) sees this and bubbles it up
    so the deploy fails fast rather than silently writing plaintext PHI.
    """
...
def _get_cipher() -> Optional[MultiFernet]:
    ...
    In production, missing or invalid config raises EncryptionConfigError so
    plaintext PHI is never written. In dev/CI a None return means the caller
    should fall through to plaintext, with a one-shot warning to keep local
    dev quiet without hiding misconfig.
    """
    ...
    try:
        ...
        if prod:
            raise EncryptionConfigError(
                f"ENCRYPTION_KEY failed to parse: {e}"
            ) from e
        logger.error(
            "ENCRYPTION_KEY invalid, falling back to plaintext in non-prod: %s", e
        )
        return None
...
def verify_encryption_configured() -> None:
    ...
    Call from ``app.main`` lifespan to surface misconfiguration on boot
    rather than at first PHI write (which can be hours after deploy).
    """
    _get_cipher()
