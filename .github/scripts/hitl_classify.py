...
# so a credential (*.p8 / *.pem / .env) accidentally committed under
# .github/workflows/ is still caught.
#
# Why: workflow YAML and hitl-config itself legitimately reference guard
# strings like EVIDENCE_BOUND_SYSTEM_PROMPT and ml_shadow_ as metadata
# (workflow prompts enumerate the refuse list, hitl-config defines the
# needles). Those mentions describe the guard; they do not flip a shadow
# flag. PR #44 was incorrectly closed as Tier 3 because it added a
# workflow whose prompt named those strings.
